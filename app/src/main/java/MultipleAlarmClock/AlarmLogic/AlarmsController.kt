...
//            var idOfAlarmInserted: Deferred<Long>? = null
            val insertedAlarmData: AlarmData = when (alarm) {
                is AlarmValueForAlarmSeries.AlarmDataType -> {
                     alarmDao.updateOrInsert(alarm.alarmData.copy(isReadyToUse = true))
                ...
                is AlarmValueForAlarmSeries.AlarmObjectType -> {
					...
					val newId = alarmDao.insert(toInsert).toInt()
            ...
            alarmResult.fold(
                onSuccess = {}, onError = { messageForUser, exception ->
					alarmDao.deleteAlarm(insertedAlarmData)
                    this@AlarmsController.cancelAlarmHandler(insertedAlarmData, activityContext, alarmManager, alarmDao)
