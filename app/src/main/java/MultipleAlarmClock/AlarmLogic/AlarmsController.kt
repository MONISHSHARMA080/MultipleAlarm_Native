...
@Inject
constructor(private val alarmRepository: AlarmRepository, private val timeProvider: TimeProvider) {
    var scope = CoroutineScope(Dispatchers.IO + SupervisorJob())
    private val alarmInfoNotificationClass: Class<out BroadcastReceiver> =
            AlarmInfoNotification::class.java
    ...
    data class PendingIntentCreated(
            /**
             * [pendingIntentToGiveUserUpcommingAlarmInfoWhenAsked] is for what happens when the
             * user clicks on the upcoming alarm notification in the notification shade, we have to
             * give info about alarm
             */
    ...
    suspend fun scheduleAlarm(
            ...
            startTimeForAlarmSeries: Long,
            alarmMessage: String = "",
            alarmData: AlarmData
    ): ResultCustom<Unit, scheduleAlarmError> {
        return ResultCustom.runCatching(defaultErrorMessage = scheduleAlarmError.GenericError()) {
            logD("the message in the startTime is $alarmMessage")
            logD(
                    " startTime:${getTimeInHumanReadableFormat(startTime)} and endTime:${getTimeInHumanReadableFormat(endTime)} \n"
            )
            ...
            if (startTime >= endTime)
                    return ResultCustom.Failure(
                            scheduleAlarmError.ProgrammerError(),
                            internalException =
                                    Exception(
                                            "the startTime:${this.getTimeInHumanReadableFormatProtectFrom0Included(startTime)} is not > endTime:${this.getTimeInHumanReadableFormatProtectFrom0Included(endTime)} "
                                    )
                    )
            if (currentCalTime > startTime)
                    return ResultCustom.Failure(
                            errorMessageToDisplayUser = scheduleAlarmError.ProgrammerError(),
                            internalException =
                                    Exception(
            ...
            if (alarmData.startTime != startTimeForAlarmSeries)
                    return ResultCustom.Failure(
                            errorMessageToDisplayUser = scheduleAlarmError.ProgrammerError(),
                            internalException =
                                    Exception(
                                            "the SeriesStartTime:${this.getTimeInHumanReadableFormatProtectFrom0Included(startTimeForAlarmSeries)} is not same as the one from the alarmData(DB):${this.getTimeInHumanReadableFormatProtectFrom0Included(alarmData.startTime)} "
                                    )
                    )
            if (alarmData.endTime != endTime)
                    return ResultCustom.Failure(
                            ...
                            internalException =
                                    Exception(
                                            "the endTime:${this.getTimeInHumanReadableFormatProtectFrom0Included(endTime)} is not same as the one from the alarmData(DB):${this.getTimeInHumanReadableFormatProtectFrom0Included(alarmData.endTime)} "
                                    )
                    )
            logD(
            ...
            val resultForAlarmOpr =
                    scope.async {
                        getPendingIntentForAlarm(
                                receiverClass,
                                componentActivity,
                                startTimeForAlarmSeries,
                                startTime,
                                endTime,
                                alarmMessage,
                                alarmData.id
            ...
            val PIForAlarm =
                    resultForAlarmOpr
                            ...
                            .fold(
                                    onSuccess = { PI -> PI },
                                    onError = { failureRes, exception ->
                                        return ResultCustom.Failure(
                                                errorMessageToDisplayUser =
                                                        scheduleAlarmError.PendingIntentNotFound(
                                                                failureRes.messageToDisplayUser
                                                        ),
                                                internalException = Exception(exception)
                                        )
                                    }
                            )
            if (PIForAlarm.pendingIntentToGiveUserUpcommingAlarmInfoWhenAsked != null)
                    return ResultCustom.Failure(
                            internalException =
                                    Exception(
                                            " the pending intent for alarm's info. notification is null "
                                    ),
                            errorMessageToDisplayUser = scheduleAlarmError.PendingIntentNotFound()
                    )
            logD("\n\n\n [INFO] the pending Intent for the alarm is $PIForAlarm ")
            // here the PI for the alarm notification
            val alarmClockInfoObject =
                    AlarmManager.AlarmClockInfo(
                            startTime,
                            PIForAlarm.pendingIntentToGiveUserUpcommingAlarmInfoWhenAsked
                    )
    ...
    fun getPendingIntentForAlarm(
            ...
            alarmMessage: String,
            alarmId: Int,
            createIntentForAlarmMetaData: Boolean = true
        ...
        return ResultCustom.runCatching(
            ...
            logD(
                    "is pendingIntent in the scheduleAlarm() null ${pendingIntentForAlarm == null}, and it is $pendingIntentForAlarm"
            )
            if (pendingIntentForAlarm != null) {
                return ResultCustom.Failure(
                        errorMessageToDisplayUser =
                                GetPendingIntentForAlarmError.PendingIntentAlreadyExist(),
                        internalException =
                                Exception(
                                        "Alarm on (${getTimeInHumanReadableFormat(startTime)}) already exists"
                                )
                )
            }
            pendingIntentForAlarm =
                    PendingIntent.getBroadcast(
                            context,
                            alarmId,
                            intent,
    ...
    /**
     * given an alarm Series(or alarm, or startTime->endTime, this function will start it. If the
     * alarm startTime is greater than current startTime then we will iterate over it and set that
     * alarm from that time
     */
    suspend fun startAlarmSeries(
            ...
            receiverClass: Class<out BroadcastReceiver> = AlarmReceiver::class.java,
            incrementTheStartTimeIfLessThanCurrentTime: Boolean = true
        ...
        return ResultCustom.runCatching(
            ...
            if (!alarmValidationResult.isValid)
                    return ResultCustom.Failure(
                            errorMessageToDisplayUser = StartAlarmSeriesError.ProgrammerError(),
                            internalException = Exception(alarmValidationResult.errorMessage)
                    )
            val currentTIme = Calendar.getInstance()
            if (alarmData.endTime < currentTIme.timeInMillis) {
                val msg =
                        "Expected the endTIme to be less than current time but got endTIme:${getTimeInHumanReadableFormat(alarmData.endTime)}, currentTime:${getTimeInHumanReadableFormat(currentTIme.timeInMillis)}"
                return ResultCustom.Failure(
                        errorMessageToDisplayUser = StartAlarmSeriesError.ProgrammerError(),
                        internalException = Exception(msg)
                )
            }
            val alarmObj = alarmData.toDomain()
            val timeReturned =
                    alarmObj.alarmTimeSequence().firstOrNull { it > currentTIme.timeInMillis }
                            ?: return ResultCustom.Failure(
                                    StartAlarmSeriesError.ErrorSchedulingAlarm(),
                                    Exception(
                                            "Can't get first alarm to start the series\n alarmData:$alarmData and converted to alarmObject is $alarmObj "
                                    )
                            )
            val scheduleAlarmHandler =
                    scope.async {
                        scheduleAlarm(
                                timeReturned,
                                alarmData.endTime,
            ...
            val lastIntentHandler =
                    scope.async {
                        this@AlarmsController
                                .lastPendingIntentWithMessageForDbOperationsWillFireAtEndTime(
                                        alarmData.startTime,
                                        activityContext,
                                        alarmManager,
            ...
            scheduleAlarmHandler
                    .await()
                    .fold(
                            onSuccess = {},
                            onError = { errorMessageForUser, exception ->
                                return ResultCustom.Failure(
                                        errorMessageToDisplayUser =
                                                StartAlarmSeriesError.ErrorSchedulingAlarm(
                                                        errorMessageForUser.messageToDisplayUser
                                                ),
                                        internalException = Exception(exception)
                                )
                            }
    ...
    /**
     * handle setting the alarms and if fails then cancel it and update the DB state not running
     * else running
     */
    suspend fun startAlarmSeriesHandler(
            alarm: AlarmValueForAlarmSeries,
            alarmManager: AlarmManager,
            activityContext: Context,
            receiverClass: Class<out BroadcastReceiver> = AlarmReceiver::class.java,
            incrementTheStartTimeIfLessThanCurrentTime: Boolean = true
        ...
        return ResultCustom.runCatching(
            ...
            val insertedAlarmData: AlarmData =
                    when (alarm) {
                        is AlarmValueForAlarmSeries.AlarmDataType -> {
                            alarmRepository.upsertAlarm(alarm.alarmData.copy(isReadyToUse = true))
                            alarm.alarmData.copy(isReadyToUse = true)
                        }
                        is AlarmValueForAlarmSeries.AlarmObjectType -> {
                            // here the ID = 0 cause it's generated by the DB and I need a value
                            // here to convert it to AlarmData(other db is also there but I prefer
                            // this)
                            val toInsert =
                                    alarm.alarmObject.toAlarmData(id = 0, isReadyToUse = true)
                            val newId = alarmRepository.saveAlarm(toInsert).toInt()
                            toInsert.copy(id = newId)
                        }
                    }
            val alarmResult =
                    this@AlarmsController.startAlarmSeries(
                            insertedAlarmData,
                            alarmManager,
                            activityContext,
                            receiverClass,
            ...
            alarmResult.fold(
                    onSuccess = {},
                    onError = { messageForUser, exception ->
                        alarmRepository.deleteAlarm(insertedAlarmData)
                        this@AlarmsController.cancelAlarmHandler(
                                insertedAlarmData,
                                activityContext,
                                alarmManager
                        )
                        return ResultCustom.Failure(
                                errorMessageToDisplayUser =
                                        StartAlarmSeriesHandlerError.ErrorSchedulingAlarm(
                                                messageForUser.messageToDisplayUser
                                        ),
    ...
    suspend fun cancelAlarmHandler(
        ...
        return ResultCustom.runCatching(
            ...
            // since the alarm is not in the DB we can't insert it and also we can't just inser it
            // as it wasn't there and the user did not want it
            this@AlarmsController.updateAlarmStateInDb(alarmData.copy(isReadyToUse = false))
                    .fold(
                            onSuccess = {},
                            onError = { messageToDisplayUser, exception ->
                                return ResultCustom.Failure(
                                        CancelAlarmHandlerError.ErrorDeletingAlarmFromDb(
                                                messageToDisplayUser.messageToDisplayUser
                                        ),
                                        exception
                                )
                            }
                    )
            cancelAlarm(alarmData, context, alarmManager)
                    .fold(
                            onSuccess = {},
                            onError = { errorToDisplayUser, exception ->
                                // since we can't cancel it then we should just
                                alarmRepository.upsertAlarm(alarmData.copy(isReadyToUse = false))
                                return ResultCustom.Failure(
                                        errorMessageToDisplayUser =
                                                CancelAlarmHandlerError.CancellingAlarmError(
                                                        errorToDisplayUser.messageToDisplayUser
                                                ),
                                        internalException = exception
                                )
                            }
    ...
    suspend fun deleteAlarmHandler(
        ...
        return ResultCustom.runCatching(
            ...
            // first update the Db as it is more visible to the user, and there is a race condition
            // here as we can't do it in parallel, as are accessing the db in failure in both
            // the alarm was not in the DB so return
            // since the alarm is not in the DB we can't insert it, and also we can't just insert it
            // as it wasn't there and the user did not wanted it
            val rowsAffected = alarmRepository.deleteAlarm(alarmData)
            if (rowsAffected == 0)
                    return ResultCustom.Failure(
                            DeleteAlarmHandlerError.AlarmNotInDbToDelete(),
                            internalException =
                                    Exception(
            ...
            cancelAlarm(alarmData, context, alarmManager)
                    .fold(
                            onSuccess = {},
                            onError = { messageToDisplayUser, exception ->
                                // since we can't delete it then we should just put it back and tell
                                // the user to try to cancel it again
                                logD(
                                        "error when cancelling alarm while deleting alarm is $messageToDisplayUser"
                                )
                                when (messageToDisplayUser) {
                                    ...
                                    is CancelAlarmError.GenericError -> {
                                        alarmRepository.upsertAlarm(
                                                alarmData.copy(isReadyToUse = false)
                                        )
                                    }
                                    is CancelAlarmError.ProgrammerError -> {
                                        //  error exist since we let there be a state in db(/app)
                                        // that should not be and now when the user wants to delete
                                        // it then we should let them and not insert it over here
                                    }
                                }
                                return ResultCustom.Failure(
                                        errorMessageToDisplayUser =
    ...
    suspend fun cancelAlarm(
        ...
        return ResultCustom.runCatching(defaultErrorMessage = CancelAlarmError.GenericError()) {
            ...
            if (!validationResult.isValid)
                    return ResultCustom.Failure(
                            CancelAlarmError.ProgrammerError(),
                            internalException = Exception(validationResult.errorMessage)
                    )
            val alarmObj = alarmData.toDomain()
            alarmObj.alarmTimeSequence().dropWhile { it > currentTime }.take(5).forEach {
                    alarmIterVal ->
                logD(
                        "the time value gotten in iterating is ${getTimeInHumanReadableFormatProtectFrom0Included(alarmIterVal)}"
                )
            ...
            // now try to mark it as not ready to use in db and we done if problem then reschedule
            // it and return error
    ...
    private inline fun cancelPendingIntentReceiver(
            baseIntent: Intent,
            context: Context,
            intentData: AlarmActivityIntentData,
            alarmReceiverClass: Class<out BroadcastReceiver>,
            alarmManager: AlarmManager,
    ...
    suspend fun updateAlarmStateInDb(
        ...
        return ResultCustom.runCatching(defaultErrorMessage = UpdateAlarmInDbError.GenericError()) {
            val rowsAffected = alarmRepository.updateAlarm(alarmData)
            if (rowsAffected == 0)
                    return ResultCustom.Failure(
                            UpdateAlarmInDbError.NoAlarmUpdated(),
                            Exception("No rows affected for alarmData:$alarmData")
                    )
        }
    }
    ...
    suspend fun rescheduleAlarm(
        ...
        return ResultCustom.runCatching(defaultErrorMessage = RescheduleAlarmError.GenericError()) {
            // should probably make some checks like if the user ST->11:30 pm today and end time 1
            // am tomorrow (basically should be in a day)
            // we can't get it form the alarmData as this func is for the reset alarm and that could
            // be only one
            ...
            if (this.getDateForDisplay(nextAlarmInfo.newSeriesStartTime) !=
                            this.getDateForDisplay(nextAlarmInfo.newSeriesEndTime)
            )
                    return ResultCustom.Failure(
                            RescheduleAlarmError.ProgrammerError(),
                            internalException =
                                    Exception(
                                            " startDate from new startSeries time is ${this.getDateForDisplay(nextAlarmInfo.newSeriesStartTime)} and the end date from the new endSeries time is ${this.getDateForDisplay(nextAlarmInfo.newSeriesEndTime)}"
                                    )
                    )
            ...
            try {
                ...
                val alarmSchedule =
                        scope.async {
                            scheduleAlarm(
                                    nextAlarmInfo.nextAlarmTriggerTime,
                                    nextAlarmInfo.newSeriesEndTime,
                                    alarmManager,
                                    activityContext,
                                    receiverClass = receiverClass,
                                    startTimeForAlarmSeries = nextAlarmInfo.newSeriesStartTime,
                                    alarmMessage = alarmData.message,
                                    alarmData =
                                            newAlarm.copy(
                                                    startTime = nextAlarmInfo.newSeriesStartTime,
                ...
                alarmSchedule
                        ...
                        .fold(
                                ...
                                onError = { errorToDisplayUser, exception ->
                                    ...
                                    return ResultCustom.Failure(
                                            errorMessageToDisplayUser =
                                                    RescheduleAlarmError.AlarmScheduleError(
                                                            errorToDisplayUser.messageToDisplayUser
                                                    ),
                                            internalException = Exception(exception)
                                    )
                ...
                // if we have gotten an error then we will need to cancel the
                // alarmcancelAlarmByCancelingPendingIntent and return the exception and also delete
                // the alarm
                this.cancelAlarm(
                        alarmDataForDeleting,
                        activityContext,
                        alarmManager,
                )
                logD(
                        "error occurred in the schedule multiple alarms, so we are going to cancel the alarm whole, in scheduleAlarm2-->${e}"
                )
                logD(
                        "[UNEXPECTED ERROR] in reschedule alarm failed to account for exception/error (exception here) -> $e"
                )
                return ResultCustom.Failure(
                        errorMessageToDisplayUser = RescheduleAlarmError.GenericError(),
                        internalException = e
                )
            }
        }
    ...
    suspend fun resetAlarms(
        ...
        return ResultCustom.runCatching(defaultErrorMessage = ResetAlarmError.GenericError()) {
            ...
            val nextAlarmInfo =
                    res.fold(
                            ...
                            onError = { errorMessageToShowUser, throwable ->
                                return ResultCustom.Failure(
                                        ResetAlarmError.CalculateNextAlarmError(
                                                errorMessageToShowUser.messageToDisplayUser
                                        ),
                                        internalException = Exception(throwable)
                                )
                            }
                    )
            if (this.getDateOnly(nextAlarmInfo.newSeriesStartTime) !=
                            this.getDateOnly(nextAlarmInfo.newSeriesEndTime)
            )
                    return ResultCustom.Failure(
                            errorMessageToDisplayUser = ResetAlarmError.ProgrammerError(),
                            internalException =
                                    Exception(
            ...
            if (this.getDateForDisplay(nextAlarmInfo.newSeriesStartTime) !=
                    ...
                    return ResultCustom.Failure(
                            ResetAlarmError.ProgrammerError(),
                            internalException =
                                    Exception(
                                            " startDate from new startSeries time is ${this.getDateForDisplay(nextAlarmInfo.newSeriesStartTime)} and the end date from the new endSeries time is ${this.getDateForDisplay(nextAlarmInfo.newSeriesEndTime)}"
                                    )
            ...
            val updatingAlarmStateJob =
                    this.scope.async { this@AlarmsController.updateAlarmStateInDb(newAlarm) }
            val scheduleAlarmJob =
                    scope.async {
                        scheduleAlarm(
                                nextAlarmInfo.nextAlarmTriggerTime,
                                nextAlarmInfo.newSeriesEndTime,
            ...
            scheduleAlarmJob
                    ...
                    .fold(
                            onSuccess = {},
                            onError = { messageToDisplay, exception ->
                                logD(
                                        "the scheduleAlarm() failed with exception ${exception.message} and message ${messageToDisplay.messageToDisplayUser} here cancelling and returning"
                                )
                                ...
                                cancelAlarm(
                                        ...
                                        activityContext,
                                        alarmManager
                                ) // if this fails then god help us
                                return ResultCustom.Failure(
                                        ResetAlarmError.SchedulingAlarmError(
                                                messageToDisplay.messageToDisplayUser
                                        ),
                                        exception
                                )
                            }
    ...
    private fun calculateNextAlarmInfo(
        ...
        return ResultCustom.runCatching(
            ...
            if (originalSeriesStart >= originalSeriesEnd)
                    return ResultCustom.Failure(
                            CalculateNextAlarmInfo.ProgrammerError(),
                            Exception(
                                    "The startTime: ${this@AlarmsController.getTimeInHumanReadableFormatProtectFrom0Included(originalSeriesStart)} is not less than endTime: ${this.getTimeInHumanReadableFormatProtectFrom0Included(originalSeriesEnd)} "
                            )
                    )
            when {
                now > originalSeriesStart && now > originalSeriesEnd -> {
                    ...
                    val startCalendar =
                            Calendar.getInstance().apply { timeInMillis = originalSeriesStart }
                    startCalendar.apply {
                    ...
                    // if we set the alarm at 3:00 - 3:10 Pm and today it is 4:00Pm so I want it to
                    // be 3:00Pm of tomorrow
                    if (startCalendar.timeInMillis < now) {
                        logD(
                                "moved the startTIme:${getTimeInHumanReadableFormat(startCalendar.timeInMillis)} to today but it is less than currentTime:${getTimeInHumanReadableFormat(now)}, so changed it to tomorrow"
                        )
                    ...
                    logD(
                            "in now > seriesStart && now > seriesEnd and the updated value is $nextAlarm"
                    )
                ...
                now < originalSeriesStart && now < originalSeriesEnd -> {
                    ...
                    logD(
                            "in the now < originalSeriesStart && now < originalSeriesEnd  and the value of upcoming alarm is same -> $nextAlarm "
                    )
                    return@runCatching nextAlarm
                }
                now in originalSeriesStart..originalSeriesEnd -> {
                    // --- Scenario 3: Series is entirely in the past (or no slots were left in
                    // Scenario 2) ---
                    ...
                    val nextAlarm =
                            NextAlarmInfo(
                                    ...
                                    newSeriesStartTime =
                                            originalSeriesStart, // The series bounds DO NOT change
                                    newSeriesEndTime = originalSeriesEnd,
                                    this@AlarmsController
                            )
                    ...
                    return ResultCustom.Failure(
                            CalculateNextAlarmInfo.IllegalStateError(),
                            Exception(
    ...
    fun lastPendingIntentWithMessageForDbOperationsWillFireAtEndTime(
            alarm_start_time_to_search_db: Long,
            context_of_activity: Context,
            alarmManager: AlarmManager,
            message_name_for_start_time: String,
            message_name_for_end_time: String,
            alarm_end_time_to_search_db: Long,
            broadcastReceiverClass: BroadcastReceiver
    ) {
        val intent = Intent(context_of_activity, broadcastReceiverClass::class.java)
        // probably should hardcode message_name_for_start_time to be alarm_end_time_to_search_db
        // and same for message_name_for_end_time
